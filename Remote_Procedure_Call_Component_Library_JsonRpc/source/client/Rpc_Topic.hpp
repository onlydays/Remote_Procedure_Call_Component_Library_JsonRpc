        ...
        class TopicManager
        {
            public:
                ...
                void onPublish(const BaseConnection::ptr& conn, const TopicRequest::ptr& msg)
                {
                    ...
                    //3.通过主体名称，查看找对应主体的回调处理函数，有则处理，无则报错
                    auto callback = getSubscribe(topic_key);
                    if(!callback){
                        DLOG("收到了 %s 主题消息，但该消息无主题处理回调！",topic_key.c_str());
                        return ;
                    }
            ...
            private:
                void addSubscribe(const std::string& key,const SubCallback& cb)
                {
                    ...
                    _topic_callbacks.insert(make_pair(key,cb));
                }
                ...
                bool commonRequest(const BaseConnection::ptr& conn,const std::string& key, TopicOptype optype, const std::string& msg = "")
                {
                    //1.构造请求对象，并填充数据
